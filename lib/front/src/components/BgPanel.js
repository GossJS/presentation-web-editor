...
import {
  getLabels,
  getInitialSlideBgSettings,
  getNewSlideBgSettings,
  getLeftPanelStatus,
} from '../selectors';
...
class BgPanel extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      Boolean(nextProps.initial) &&
			Object.keys(nextProps.initial).length
    ) {
      const nextState = Object.assign({}, nextProps.initial);
  ...
  handleSubmit(e) {
    const stringsToInsert = Object.keys(this.changedSettings).map((key) => {
      // if (this.changedSettings[key] === '') {
      // return null;
      // }

      const insert = this.changedSettings[key]
        ? `${key}="${this.changedSettings[key]}"`
        : '';
      const pattern = `${key}="[^"]+"`;
  ...
  updateBgPos(e, axis) {
    const currentBgPos = this.state['background-position'].split(' ');
    const newBbPosBit = e.target.value;
    const newBgPos = currentBgPos;
    const updateIndex = axis === 'x' ? 0 : 1;

    newBgPos[updateIndex] = newBbPosBit;
    Object.assign(this.changedSettings, {
      'background-position': newBgPos.join(' '),
    });
  }
  ...
  render() {
    return (
      <Panel
        name="leftPanel bgOptionsPanel"
        closePanel={this.props.closePanel}
      >
        <InputWrapper name="colorWrapper">
          <Label
            inputId="colorBg"
            text={this.props.labels['Background color']}
          />
          <input
            id="colorBg"
            type="color"
        ...
        <InputWrapper name="bgSizeWrapper">
          <Label
            inputId="bgSize"
            text={this.props.labels['Background size']}
          />
        ...
        <InputWrapper name="bgPositionXWrapper">
          <Label
            inputId="bgPositionX"
            text={this.props.labels['Background position x']}
          />

          <select
            id="bgPositionX"
            className="bgPositionPartial"
        ...
        <InputWrapper name="bgPositionYWrapper">
          <Label
            inputId="bgPositionY"
            text={this.props.labels['Background position y']}
          />
        ...
        <InputWrapper name="bgRepeatWrapper">
          <Label
            inputId="bgRepeat"
            text={this.props.labels['Background repeat']}
          />
          <select
            id="bgRepeat"
            onChange={e =>
							this.handleChange(e, 'background-repeat')
						}
          >
        ...
        <InputWrapper name="videoBgLoopWrapper">
          <input
            ...
            onChange={e =>
							this.handleChange(e, 'background-video-loop')
						}
          />
          <Label
            inputId="videoBgLoop"
            text={this.props.labels['Background video loop']}
          />
        </InputWrapper>
        ...
        <InputWrapper name="videoBgMutedWrapper">
          <input
            ...
            onChange={e =>
							this.handleChange(e, 'background-video-muted')
						}
          />
          <Label
            inputId="videoBgLoop"
            text={this.props.labels['Background video muted']}
          />
        </InputWrapper>
        ...
        <InputWrapper name="iframeBgWrapper">
          <Label
            inputId="iframeBg"
            text={this.props.labels['Background web page']}
          />
          <input
            id="iframeBg"
            type="text"
            ...
            onChange={e =>
							this.handleChange(e, 'background-iframe')
						}
            value={this.state['background-iframe']}
          />
          <Button
            data-input-id="iframeBg"
            text={this.props.labels.Reset}
            clickHandler={() =>
							this.recordChange({ 'background-iframe': '' })
						}
          />
        </InputWrapper>
