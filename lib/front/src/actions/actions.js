...
export function insert(arg1, arg2) {
  const stringsToInsert = Array.isArray(arg1)
    ? arg1
    : [{ insert: arg1, pattern: arg2 }];
  return { type: 'INSERT', stringsToInsert };
}

export function insertImage() {
  return function (dispatch) {
    const filePathRequest = mainProcess.getFilePath('img');

    if (!filePathRequest) return;
...
export function openSlideBgPanel() {
  return function (dispatch, getState) {
    const state = getState();
    const currentSettings = getCurrentSlideBgSettingsFromEditor(getEditorData(state));
...
export function setTransition(transitionName) {
  return {
    ...
    newConf: { transition: transitionName },
  };
}
